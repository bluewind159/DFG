...
import numpy as np
from .graph_label import GraphEncoder
    ...
    def forward(
        ...
        if labels is not None:
            ...
            if adv_trans is not None:
                ...
                label_emb = self.graph_encoder(labels, lambda x: self.bert.embeddings(x)[0])
                recons_result=recons(neg_temp,target,label_emb)
                ...
                for ww in range(self.num_labels):
                    logits_final.append(self.classifier_single[ww](pos_temp[:,ww,:]))
