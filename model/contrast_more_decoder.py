...
class ContrastModel(BertPreTrainedModel):
    def __init__(self, config, cls_loss=True, contrast_loss=True, graph=False, layer=1, data_path=None,
        ...
        for rr in range(self.num_labels):
            self.classifier_single.append(nn.Linear(config.hidden_size, 1))
        self.classifier_single2 = nn.ModuleList()
        for rr in range(self.num_labels):
            self.classifier_single2.append(nn.Linear(config.hidden_size, 1))
        ...
        if labels is not None:
            ...
            if adv_trans is not None:
                ...
                for kk in range(self.num_labels):
                    logits_split.append(self.classifier_single[kk](pos_temp[:,kk,:]))
                ...
                for ww in range(self.num_labels):
                    logits_final.append(self.classifier_single2[ww](final_features[:,ww,:]))
                print('different decoder')
                assert 0==1
