...
class ContrastModel(BertPreTrainedModel):
    def __init__(self, config, cls_loss=True, contrast_loss=True, graph=False, layer=1, data_path=None,
        ...
        self.classifier_single = nn.ModuleList()
        for i in range(self.num_labels):
            self.classifier_single.append(nn.Linear(config.hidden_size, 1))
        ...
        self.label_tree = torch.load(os.path.join(data_path, 'slot.pt'))
        self.lamb = lamb
    ...
    def forward(
        ...
        if labels is not None:
            ...
            ################################################################################################################################
            if adv_trans is not None:
                pos_temp=[]
                neg_temp=[]
                
                label_h=get_label_degree(self.label_tree)
                anti_tree={}
                for key_parent,value_parent in self.label_tree.items():
                    for value in value_parent:
                        anti_tree[value]=key_parent
                split_h=2 #from [0,1,2,3]
                acc=[]
                number=0
                ...
                layer_count=number
                all_features_pos=torch.zeros(pooled_output.size(0),self.num_labels,pooled_output.size(-1)).cuda()
                all_features_neg=torch.zeros(pooled_output.size(0),self.num_labels,pooled_output.size(-1)).cuda()
                disen_loss=0
                for i,layer in enumerate(adv_trans[:layer_count]):
                    pos_qwer,neg_qwer=layer(pooled_output)
                    disen_loss+=self.MI(torch.cat([pos_qwer,neg_qwer],dim=0))/layer_count
                    all_features_pos[:,acc[i],:]=pos_qwer
                    all_features_neg[:,acc[i],:]=neg_qwer
                ...
                for keys,values in label_h.items():
                    if keys > split_h:
                        for value in values:
                            all_features_pos[:,value,:]=all_features_pos[:,new_anti_tree[value],:]
                            all_features_neg[:,value,:]=all_features_neg[:,new_anti_tree[value],:]
                #################### test            
                #all_features=torch.zeros(self.num_labels)
                #count=0
                #print(self.label_tree)
                ...
                #for keys,values in label_h.items():
                    #if keys>split_h:
                        #for value in values:
                            #all_features[value]=all_features[new_anti_tree[value]]
                #print(all_features)
                #assert 0==1
                #######################################################            
                logits_split=[]
                for kk in range(self.num_labels):
                    logits_split.append(self.classifier_single[kk](all_features_pos[:,kk,:]))
                logits_split=torch.cat(logits_split,dim=-1).unsqueeze(-1)
                ...
                #final_features = self.graph_split233(final_features, labels, lambda x: self.bert.embeddings(x)[0])
                logits_final=[]
                for ww in range(self.num_labels):
            ...
            if self.cls_loss:
                if self.num_labels == 1:
                    ...
                    loss_fct = MSELoss()
                    loss += layer_count*loss_fct(logits.view(-1), labels.view(-1))
                else:
                    loss += layer_count*loss_fct(logits.view(-1, self.num_labels), target)
        ...
        return {
            'loss': loss,
            'logits': logits_final[:,:,0],
            'hidden_states': outputs.hidden_states,
            'attentions': outputs.attentions,
            'contrast_logits': contrast_logits,
            'local': pooled_output,
            'pos_temp':all_features_pos,
            'neg_temp': all_features_neg,
            'target': target,
            'embeddings': lambda x: self.bert.embeddings(x)[0]
        }
