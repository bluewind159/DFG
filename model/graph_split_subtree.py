...
class GraphEncoder(nn.Module):
    def __init__(self, config, graph=False, layer=1, data_path=None, threshold=0.01, tau=1):
        ...
        self.hir_layers = nn.ModuleList([GraphLayer(config, last=False) for i in range(layer)])
        ...
        if graph:
            label_tree = torch.load(os.path.join(data_path, 'slot.pt'))
            label_h=get_label_degree(label_tree)
            split_h=3 #from [0,1,2,3]
            print('split hier is',split_h)
            new_label_tree={}
            for i in range(split_h):
                values=label_h[i]
                for value in values:
                    if len(label_tree[value])>0:
                        new_label_tree[value]=label_tree[value]
            label_hier=new_label_tree
    ...
    def forward(self, inputs_embeds, labels, embeddings):
        label_emb = inputs_embeds
        label_attn_mask = torch.ones(1, label_emb.size(1), device=label_emb.device)
        extra_attn = None

        self_attn_mask = (label_attn_mask * 1.).t().mm(label_attn_mask * 1.).unsqueeze(0).unsqueeze(0).repeat(label_emb.size(0),1,1,1)
        expand_size = label_emb.size(-2) // self.label_num
        if self.graph:
            if GRAPH == 'GRAPHORMER':
                id_emb = self.id_embedding(self.label_id[:, None].expand(-1, expand_size)).view(1, -1, self.config.hidden_size)
