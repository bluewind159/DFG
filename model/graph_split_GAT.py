...
class GraphLayer(nn.Module):
    ...
    def forward(self, label_emb, extra_attn, self_attn_mask):
        if GRAPH == 'GRAPHORMER':
            ...
            acc=[]
            for i in range(label_emb.size(0)):
                qwe = self.hir_attn(label_emb[i], edge_index=extra_attn)
                acc.append(qwe.unsqueeze(0))
            label_emb = torch.cat(acc)
        label_emb = self.output_layer_norm(self.dropout(self.output_layer(label_emb)) + label_emb)
        return label_emb
...
class GraphEncoder(nn.Module):
    def __init__(self, config, graph=False, layer=1, data_path=None, threshold=0.01, tau=1):
        ...
        print('Only use GAT as the GraphEncoder')
