...
class ContrastModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        logits_final=logits.unsqueeze(-1)
        if adv_trans is not None:
            ...
            logits_single=self.classifier_single(pos_temp)
        ...
        if writer is not None:
            ...
            count=0
            target=labels
            #for key,values in self.label_tree.items():
            for yy in range(pos_temp.size(0)):
                #if target[yy,key]>0:
                #    new_pos_temp.append(pos_temp[yy,key,:].unsqueeze(0))
                #    new_final_features.append(final_features[yy,key,:].unsqueeze(0))
                #    print_labels.append(count)
                
                for value in range(pos_temp.size(1)):
                #for value in values:
                    if target[yy,value]>0:
                        new_pos_temp.append(pos_temp[yy,value,:].unsqueeze(0))
                        new_final_features.append(final_features[yy,value,:].unsqueeze(0))
                        print_labels.append(value)
            new_pos_temp=torch.cat(new_pos_temp,dim=0)
            new_final_features=torch.cat(new_final_features,dim=0)
            new_print_labels=torch.LongTensor(print_labels)
            writer.add_embedding(new_pos_temp, print_labels, label_img=None, global_step=0, tag='disentangle_feature')
            writer.add_embedding(new_final_features, print_labels, label_img=None, global_step=0, tag='GraphEncoder_output')
            assert 0==1
