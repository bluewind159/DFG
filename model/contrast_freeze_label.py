    ...
    def forward(
        ...
        if labels is not None:
            ...
            if adv_trans is not None:
                ...
                disen_loss=0
                recons_loss=0
                label_emb = self.graph_encoder(target,lambda x: self.bert.embeddings(x)[0])
                label_emb = label_emb*target.unsqueeze(-1)
                for rr,layer in enumerate(adv_trans[:layer_count]):
                    pos_qwer,neg_qwer=layer(pooled_output)
                    disen_loss += self.MI(torch.cat([pos_qwer,neg_qwer],dim=0))/layer_count
                    recons_loss+=-self.MI(torch.cat([neg_qwer,label_emb[:,rr,:]],dim=0))/layer_count
