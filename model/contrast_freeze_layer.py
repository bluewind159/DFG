...
class ContrastModel(BertPreTrainedModel):
    def __init__(self, config, cls_loss=True, contrast_loss=True, graph=False, layer=1, data_path=None,
        ...
        self.graph_split233 = GraphSplitEncoder(config, graph, layer=4, data_path=data_path)
        self.lamb = lamb
    ...
    def forward(
        ...
        if labels is not None:
            ...
            if adv_trans is not None:
                ...
                #final_features = self.graph_split456(final_features, labels, lambda x: self.bert.embeddings(x)[0])
