    ...
    def forward(
        ...
        if labels is not None:
            ...
            if adv_trans is not None:
                ...
                logits_all=[]
                split_result=[]
                for layer in adv_trans:
                    if 'Linear' in str(layer):
                        layer_count+=1
                    else:
                        break
                for layer in adv_trans[:layer_count]:
                    qwer=layer(pooled_output)
                    temp.append(qwer.unsqueeze(0))
                    split_result.append(qwer)
                temp=torch.cat(temp)#.transpose(1,0)
                selected_pred=gumbel_sigmoid((adv_trans[-2](temp,temp)-0.5),0.3)
                selected_idx= (selected_pred>=0.5).byte().float()
                selected = selected_pred + (selected_idx - selected_pred).detach()
                final_result=(selected*temp).sum(0)
                logits_final = self.classifier(final_result)
                logits_all.append(logits_final)
                for i in range(temp.size(0)):
                    acc=temp[:i+1].sum(0)
                    u_acc=temp[i+1:].sum(0)
                    loss_pos = self.MI( torch.cat([acc, pooled_output],dim=0))
                    loss_neg = self.MI( torch.cat([u_acc, pooled_output],dim=0))
                    loss_sim += 1e-4/((i+1))*(-loss_pos) + 1e-4/((i+1))*(loss_neg)
                    split_features.append(acc)
                    u_split_features.append(u_acc)
                    logits_split = self.classifier(acc)
                    if labels is not None:
                        loss_add=loss_fct(logits_split.view(-1, self.num_labels), target)
                    else:
                        loss_add=0
                    loss_split+=loss_add
                loss_split=loss_split+loss_sim
                #print('split::',loss_split)
                loss += loss_split
                #print('loss1::',loss)
                #num=30， 在1e-4和1e-4的情况下可以到81.42的87.31，但是还不够，需要更好的结果
                #果然，上面的参数在多跑几个epoch之后结果到了81.40的87.61以及81.45的87.53
                #num=40其余不变的情况下可以到81.37,87.43
            ########################################################################################   
            if self.cls_loss:
                if self.num_labels == 1:
                    ...
                    #print('loss2::',loss)
                    loss_final=loss_fct(logits_final.view(-1, self.num_labels), target)
                    #print('final::',loss_final)
                    loss += layer_count*loss_final
                    #print('loss3::',loss)
                else:
                    loss += layer_count*loss_fct(logits.view(-1, self.num_labels), target)
                    #print('loss4::',loss)
                    loss_final=loss_fct(logits_final.view(-1, self.num_labels), target)
                    #print('final::',layer_count*loss_final)
                    loss += layer_count*loss_final
                    #print('loss5::',loss)
        ...
        return {
            ...
            'u_split_features': u_split_features,
            'all_split_features': torch.cat(split_result)
        }
