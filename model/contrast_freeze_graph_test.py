...
class ContrastModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        if adv_trans is not None:
            ...
            final_features = adv_trans[-2](pos_temp, labels, lambda x: self.bert.embeddings(x)[0])
            final_features = adv_trans[-1](final_features, labels, lambda x: self.bert.embeddings(x)[0])
