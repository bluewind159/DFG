...
class BertDataset(Dataset):
    def __init__(self, max_token=492, device='cpu', pad_idx=0, data_path=None):
        self.device = device
...
if __name__ == '__main__':
    ...
    from get_recons_model_indenpendent import get_recons_model
    ...
    model_recons,optimizer_recons=get_recons_model(args,size1=768,data_path=data_path,embeddings=copy.deepcopy(model.bert.embeddings))
    ...
    test = DataLoader(test, batch_size=args.batch, shuffle=False, collate_fn=dataset.collate_fn)
    model.to(device)
    ...
    for epoch in range(1000):
        ...
        for data, label, idx in pbar:
            ...
            if i % args.update == 0:
                ...
                i = 0
                loss = 0
            recons_result=model_recons(output['neg_temp'].detach(),output['target'].detach())
