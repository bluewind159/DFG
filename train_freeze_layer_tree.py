...
from model.contrast_freeze_layer_tree import ContrastModel
...
if __name__ == '__main__':
    ...
    checkpoint = torch.load('./checkpoints/rcv1-test_layer_tree/checkpoint_best_macro.pt')
    ...
    for n,p in model.named_parameters():
        if 'graph_split' in n or 'classifier_single' in n:
            continue
        else:
            p.requires_grad=False
    param_freeze=[p for n,p in model.named_parameters() if p.requires_grad]
    if args.warmup > 0:
        optimizer = ScheduledOptim(Adam(param_freeze,
                                        lr=args.lr), args.lr,
                                   n_warmup_steps=args.warmup)
    else:
        optimizer = Adam(param_freeze,
                         lr=args.lr)
    #if args.warmup > 0:
    ...
    best_score_macro = 0
    best_score_micro = 0
    best_score_macro_split = 0
    best_score_micro_split = 0
    early_stop_count = 0
    print('Use the recons target!!!!!!')
    ...
    for epoch in range(1000):
        ...
        for data, label, idx in pbar:
            ...
            recons_loss=torch.nn.MSELoss()(recons_result,output['recons_target'].detach())
        ...
        with torch.no_grad():
            ...
            for data, label, idx in pbar:
                ...
                for l233 in output['logits_split']:
                    pred_split.append(torch.sigmoid(l233).tolist())

        pbar.close()
        scores_split = evaluate(pred_split, truth, label_dict)
        macro_f1_split = scores_split['macro_f1']
        micro_f1_split = scores_split['micro_f1']
        print('pred_split:','macro', macro_f1_split, 'micro', micro_f1_split)
        
        scores = evaluate(pred, truth, label_dict)
        macro_f1 = scores['macro_f1']
        micro_f1 = scores['micro_f1']
        print('pred_final:','macro', macro_f1, 'micro', micro_f1)
        ...
        if macro_f1_split > best_score_macro_split:
            best_score_macro_split = macro_f1_split
            save(macro_f1_split, best_score_macro_split, os.path.join('checkpoints', args.name, 'checkpoint_best_macro_split.pt'))
            save_gen(macro_f1_split, best_score_macro_split, os.path.join('checkpoints', args.name, 'checkpoint_best_macro_gen_split.pt'))
            early_stop_count = 0

        if micro_f1_split > best_score_micro_split:
            best_score_micro_split = micro_f1_split
            save(micro_f1_split, best_score_micro_split, os.path.join('checkpoints', args.name, 'checkpoint_best_micro_split.pt'))
            save_gen(micro_f1_split, best_score_micro_split, os.path.join('checkpoints', args.name, 'checkpoint_best_micro_gen_split.pt'))
            early_stop_count = 0
