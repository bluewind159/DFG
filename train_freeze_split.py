...
if __name__ == '__main__':
    ...
    checkpoint = torch.load('./checkpoints/rcv1-disentangle/checkpoint_best_macro.pt')
    ...
    for epoch in range(1000):
        ...
        for data, label, idx in pbar:
            ...
            neg_temp=[]
            disen_loss=0
            layer_count=model.num_labels
            loss_fct = torch.nn.BCEWithLogitsLoss()
            target = label.to(torch.float32)
            for layer in model_gen[:layer_count]:
                pos_qwer,neg_qwer=layer(output['local'].detach())
                disen_loss+=model.MI(torch.cat([pos_qwer,neg_qwer],dim=0))/layer_count
            ...
            logits_split=model.classifier_single(pos_temp)
            loss_split=layer_count*loss_fct(logits_split[:,:,0], target)
            recons_result=model_recons(neg_temp,target,output['embeddings'])
            recons_loss=torch.nn.MSELoss()(recons_result,output['local'].detach().unsqueeze(1).repeat(1,recons_result.size(1),1))
            distan_loss = loss_split+layer_count*0.03*(disen_loss+recons_loss)
            distan_loss.backward()
            optimizer_gen.step()
