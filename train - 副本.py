...
if __name__ == '__main__':
    ...
    MINE=MINE_NWJ()   
    MINE2=MINE_NWJ()                         
    model_gen,tokenizer_gen,optimizer_gen=get_trans_model(args,size1=768,size2=768, num=30)
    ...
    for epoch in range(1000):
        ...
        for data, label, idx in pbar:
            ...
            output = model(data, padding_mask, labels=label, return_dict=True, adv_trans=model_gen)
            split_features=output['split_features']
            u_split_features=output['u_split_features']
            all_split_feature=output['all_split_features']
            local = output['local']
            loss_distan = MINE2.compute_MI(all_split_feature, all_split_feature.detach())
            count=0
            loss_split=0
            '''
            max_len=max(padding_mask.sum(-1))
            for j,(split_feature,u_split_feature) in enumerate(zip(split_features,u_split_features)):
                loss_pos = MINE.compute_MI( split_feature, local[:,0,:])
                loss_neg = MINE.compute_MI( u_split_feature, local[:,0,:])
                loss_split += 3e-4/((j+1))*(-loss_pos) + 3e-4/((j+1))*(loss_neg)
                count+=1
            '''
            loss /= args.update
            #output['loss'].backward()
            loss += output['loss'] - loss_distan #+ loss_split
            loss_final = output['loss'] - loss_distan #+ loss_split
            loss_final.backward()
            i += 1
            if i % args.update == 0:
                optimizer.step()
                ...
                #loss_split=0
                #loss_adv=0
            MINE2.train_step(all_split_feature.detach(), all_split_feature.detach(),scheduler_MI2)
            #for k,(split_feature,u_split_feature) in enumerate(zip(split_features,u_split_features)):
            #    MINE.train_step( split_feature.detach(), local[:,0,:].detach(),scheduler_MI)
            #    MINE.train_step( u_split_feature.detach(), local[:,0,:].detach(),scheduler_MI)     
            #if i>=200:
            #   break  
        pbar.close()
        model.eval()
