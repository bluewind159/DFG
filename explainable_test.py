...
from model.contrast_test_explain import ContrastModel
import os
...
if __name__ == '__main__':
    ...
    test = DataLoader(test, batch_size=128, shuffle=False, collate_fn=dataset.collate_fn)
    dev = Subset(dataset, split['val'])
    dev = DataLoader(dev, batch_size=512, shuffle=False, collate_fn=dataset.collate_fn)
    ...
    model_recons.eval()
    pbar = tqdm(dev)
    writer = SummaryWriter('explain/rcv1-new')
    with torch.no_grad():
        for data, label, idx in pbar:
            padding_mask = data != tokenizer.pad_token_id
            output = model(data, padding_mask, labels=label, return_dict=True, adv_trans=model_gen, recons=model_recons, writer=writer)
            for l in label:
                t = []
