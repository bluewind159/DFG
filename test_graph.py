...
from model.contrast_freeze_graph_test import ContrastModel
...
if __name__ == '__main__':
    ...
    test = DataLoader(test, batch_size=256, shuffle=False, collate_fn=dataset.collate_fn)
    dev = Subset(dataset, split['val'])
    dev = DataLoader(dev, batch_size=32, shuffle=False, collate_fn=dataset.collate_fn)
    model.load_state_dict(checkpoint['param'])

    model.to(device)
    from get_trans_model_graph import get_trans_model
    from get_recons_model_new import get_recons_model                    
    model_gen,tokenizer_gen,optimizer_gen=get_trans_model(model.config,size1=768,size2=768, num=len(label_dict), 
                                                          graph=args.graph,layer=args.layer, data_path=data_path)
