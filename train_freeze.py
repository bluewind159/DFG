...
from model.contrast_freeze import ContrastModel
...
parser.add_argument('--data', type=str, default='BGC', choices=['WebOfScience', 'BGC', 'rcv1', ], help='Dataset.')
...
if __name__ == '__main__':
    ...
    checkpoint = torch.load('./checkpoints/BGC-DBpedia_test/checkpoint_best_macro.pt')
    model_dict = model.state_dict()
    pretrained_dict = checkpoint['param']
    ...
    dev = DataLoader(dev, batch_size=16, shuffle=False, collate_fn=dataset.collate_fn)
    test = DataLoader(test, batch_size=16, shuffle=False, collate_fn=dataset.collate_fn)
    model.to(device)
