...
class recons(nn.Module):
    ...
    def forward(self, x, labels, embeddings):
        ...
        acc=torch.zeros(label_emb.size(0),len(self.label_tree),label_emb.size(-1)).cuda()
        for i, (key,values) in enumerate(self.label_tree.items()):
            acc[:,i,:]+=label_emb[:,key,:]
            for value in values:
                acc[:,i,:]+=label_emb[:,key,:]
            acc[:,i,:]=acc[:,i,:]/(len(values)+1)
        result=self.recons(torch.cat([x,acc],dim=-1))
        return result
